...
const generateToken=async(id)=>{
    const token=await jwt.sign({id:id},process.env.JWT_SECRET,{expiresIn:"1d"})
...
export const loginUser=async(req,res)=>{
  try {
    ...
    if(!user){
      return res.status(StatusCodes.UNAUTHORIZED).json({err:"invalid credentials"})
    }
    ...
    return res.status(StatusCodes.OK).json({msg:"u have logged in successfully"})
  } catch (err) {
    console.log(err);
    return res.status(StatusCodes.INTERNAL_SERVER_ERROR).send({ err: "internal serve error 🥹" });
  }
